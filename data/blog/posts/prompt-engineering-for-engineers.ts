...
export const post: BlogPost = {
  ...
  sections: [
    ...
    {
      kind: "p",
      text: "When you standardise on **AWS Bedrock**, you still swap foundation models behind one API—but **prompt packaging is not portable** if you pretend every model reads the same system block the same way. **Model-agnostic** layers are your contracts: JSON schema, tool definitions, task rubrics, eval cases, and **canonical** user content (no vendor-specific markup in the business payload). **Model-specific** adapters sit at the edge: for **Anthropic Claude**, structure system vs user with clear delimiters and lean on message roles and (where allowed) XML-style section tags the model was tuned on; for **Llama-family** models on Bedrock, you often lean on explicit **instruction headers**, BOS/EOS habits, and different stop-sequence behaviour. Keep adapters in small modules per model family so CI can run the same golden tests through each adapter and fail when a vendor tweak breaks one encoding path.",
    },
    {
      kind: "ul",
      items: [
